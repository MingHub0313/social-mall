...
public class AliPayTemplate {
    ...
    /**
     * 支付宝收单
     * 1.订单在支付页,不支付,一直刷新,订单过期了才支付,订单状态改为已支付,但是库存已经解锁 ===>通过支付宝的收单 设置 相对时间(timeout_express) 过期]  
     *      使用支付宝自动收单功能解决.只要字段时间不支付,就不能支付了
     * 2.由于延时等问题,订单解锁完成,正在解锁库存的时候,异步通知才到
     *      订单解锁,手动调用收单
     * 3.网络阻塞问题,订单支付成功的异步通知一直不到达
     *      查询订单列表,ajax获取当期未支付的订单状态,查询订单状态,再获取一下支付宝此订单的状态
     * 4.其它各种问题
     *      每天晚上闲时下载支付宝对账单  ---进行对账
     */
